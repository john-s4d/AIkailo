...
namespace AIkailo.Data.DBCore
{
    ...
    {
        readonly ITreeNodeManager<K, V> _nodeManager;
        readonly bool _allowDuplicateKeys;
        ...
        public IEnumerable<Tuple<K, V>> LargerThanOrEqualTo(K key)
        {
            ...
            var node = FindNodeForIteration(key, this._nodeManager.RootNode, true, ref startIterationIndex);

            return new TreeTraverser<K, V>(_nodeManager
                , node
        ...
        {
            var startIterationIndex = 0;
            var node = FindNodeForIteration(key, this._nodeManager.RootNode, false, ref startIterationIndex);
