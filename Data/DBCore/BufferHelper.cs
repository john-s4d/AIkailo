...
namespace AIkailo.Data.DBCore
{
	...
	public static class BufferHelper
	{
		...
		public static ulong ReadBufferUInt64(byte[] buffer, int bufferOffset)
		{
			var longBuffer = new byte[8];
			 Buffer.BlockCopy(buffer, bufferOffset, longBuffer, 0, 8);
			return LittleEndianByteOrder.GetUInt64(longBuffer);
		}
		...
		public static byte[] ReadBufferOffset (byte[] buffer, int bufferOffset, int length)
        {
			var resultBuffer = new byte[length];
			Buffer.BlockCopy(buffer, bufferOffset, resultBuffer, 0, length);
			return resultBuffer;
        }
