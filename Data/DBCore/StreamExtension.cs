...
namespace AIkailo.Data.DBCore
{
	public static class StreamExtension
	{
        ...
        /// <summary>
		/// Expect the upcoming 2 bytes are int16, read it and return it;
		/// </summary>
		public static short ExpectInt16(this Stream target)
        {
            var buff = new byte[2];
            if (target.Read(buff) == 2)
            {
                return LittleEndianByteOrder.GetInt16(buff);
            }
            else
            {
                throw new EndOfStreamException();
		...
		/// Expect the upcoming 2 bytes are int16, read it and return it;
		/// </summary>
		public static ushort ExpectUInt16(this Stream target)
        {
            var buff = new byte[2];
            if (target.Read(buff) == 2)
            {
                return LittleEndianByteOrder.GetUInt16(buff);
            }
            else
            {
                throw new EndOfStreamException();
        ...
        /// <summary>
        /// Expect the upcoming 8 bytes are Uint64, read it and return it;
        /// </summary>
        public static ulong ExpectUInt64(this Stream target)
        {
            var buff = new byte[8];
            if (target.Read(buff) == 8)
            {
                return LittleEndianByteOrder.GetUInt64(buff);
            }
            else
            {
                throw new EndOfStreamException();
        ...
        /// <summary>
		/// Expect upcoming 1 bytes is a byte, read and return it
		/// </summary>
		public static byte ExpectByte(this Stream target)
        {
            var buff = new byte[1];
            if (target.Read(buff) == 1)
            {
                return buff[0];
            }
            else
